      ...
      <AnimatePresence mode="wait" initial={false}>
        {state.success ? (
          ...
          <motion.form
            ...
            exit={{ opacity: 0, y: -6 }}
            transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }}
            className="flex h-12 w-full items-center overflow-hidden rounded-[var(--sf-radius-sm,4px)] glass-panel border p-1 transition-all focus-within:border-foreground focus-within:ring-2 focus-within:ring-ring/30"
          >
            <input
              type="email"
              name="email"
